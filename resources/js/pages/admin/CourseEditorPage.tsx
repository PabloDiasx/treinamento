              ...
              {videos.map((v: CourseVideo) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 truncate">{v.video_url ?? "Sem link"}</p>
                  </div>
                  <span className="flex-shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium uppercase" style={{ backgroundColor: "#6d28d915", color: "#a78bfa" }}>
                    {TAG_OPTIONS.find((t) => t.value === v.tag)?.label ?? v.tag}
                  </span>
                  <button
                    onClick={() => { setEditingVideo(v); setShowVideoForm(true) }}
